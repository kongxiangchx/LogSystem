...
void Logger::setFormatter(LogFormatter::ptr val) {
    ...
    for(auto& i : m_appenders) {
        SpinLock::SpinLockGuard guard_i(i->m_spinlock);
        if(!i->m_hasFormatter) {
...
void Logger::setFormatter(const std::string& val) {
    chx::LogFormatter::ptr new_val(new chx::LogFormatter(val));
    if(new_val->isError()) {
...
void Logger::addAppender(LogAppender::ptr appender) {
    ...
    if(!appender->getFormatter()) {
        SpinLock::SpinLockGuard guard_i(appender->m_spinlock);
...
chx::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    chx::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine>& old_value,
                    const std::set<LogDefine>& new_value) {
            CHX_LOG_INFO(CHX_LOG_ROOT()) << "on_logger_conf_changed";
            for(auto& i : new_value) {
                auto it = old_value.find(i);
                chx::Logger::ptr logger;
                if(it == old_value.end()) {
                    //新增logger
