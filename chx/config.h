#ifndef __CHX_CONFIG_H__
#define __CHX_CONFIG_H__
...
public:
    std::map<std::string, T> operator()(const std::string& v) {
        ...
        for(auto it = node.begin();
            ...
            ss.str("");
            ss << it->second;
            m.insert(make_pair(it->first.Scalar(), LexicalCast<std::string, T>()(ss.str())));
        }
        return m;
    }
};
...
public:
    std::unordered_map<std::string, T> operator()(const std::string& v) {
        ...
        for(auto it = node.begin();
            ...
            ss.str("");
            ss << it->second;
            umap.insert(make_pair(it->first.Scalar(), LexicalCast<std::string, T>()(ss.str())));
        }
        return umap;
    }
};
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            ...
            if(tmp) {
                ...
                CHX_LOG_ERROR(CHX_LOG_ROOT()) << "Lookup name=" << name << " exists but type not "
        ...
        if(name.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ._0123456789")
                != std::string::npos) {
            CHX_LOG_ERROR(CHX_LOG_ROOT()) << "Lookup name invalid " << name;
